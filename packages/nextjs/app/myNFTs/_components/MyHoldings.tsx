    ...
    const updateMyCollectibles = async (): Promise<void> => {
      ...
      for(const collection of allCollections) {
        ...
        for(let tokenIndex = 0; tokenIndex < totalBalance; tokenIndex++) {
          try {
            // const tokenId = String(await myNFTCollectionContract.read.tokenOfOwnerByIndex([
            //   connectedAddress,
            //   BigInt(tokenIndex),
            // ]));
            const tokenURI = String(await myNFTCollectionContract.read.tokenURI([BigInt(tokenIndex)]));
